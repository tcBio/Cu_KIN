...
__global__ void compute_kinship_kernel(const uint64_t* __restrict__ geno_words,
...
void compute_kinship(const uint64_t* d_genotypes,
    ...
    compute_kinship_kernel<<<grid, block, 0, stream>>>(d_genotypes,
                                                       d_similarity,
                                                       n_samples,
                                                       words_count,
                                                       inverse_norm);
    throw_if_error(cudaGetLastError(), "compute_kinship_kernel launch");
}
